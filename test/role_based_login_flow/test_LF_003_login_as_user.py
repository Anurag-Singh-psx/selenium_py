...
search=data['SEARCH']
login_details=data['USER']

@allure.epic("Challenge")
@allure.feature("Role-Based Login Flow")
@allure.story("LF_003 - Login as user")
@allure.description("Login with valid User credentials and verify User dashboard")
...
class TestLF003:
    ...
    def test_verify_error_message(self,pages):
        common_page = pages["common"]
        with allure.step("Verify user dashboard is displayed with welcome message and User role info"):
            assert_error=common_page.verify_alert_message("You are logged in as USER")
